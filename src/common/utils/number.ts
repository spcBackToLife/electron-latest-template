...
const isDecimalNumber = (val: string, decimalNumber: number) => {
    if (!val) {
        return true;
    }
...
export const checkNumberInvalid = (
    ...
    const { checkNumber, checkInRange, checkInteger, decimalNumber, endWith } = options;
    ...
    if (checkInRange && (valNum < checkInRange.min || valNum > checkInRange.max)) {
...
export const compareNumber = (x: any, y: any, operator: NumberOperator): boolean => {
...
export function isNDecimal(input: string, n: number): boolean {
    // 使用正则表达式检查输入是否为有效的小数格式
    const regex = new RegExp(`^-?\\d*\\.\\d{${n}}$`);
